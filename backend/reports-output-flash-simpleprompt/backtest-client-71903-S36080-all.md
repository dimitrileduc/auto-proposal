...
- **Appels**: 12
- **Tokens**: 9,902 input + 3,534 output = 13,436 total
...
| Métrique | Valeur | Interprétation |
|----------|--------|----------------|
| **MAE** | 12.67 unités | Erreur moyenne absolue (symétrique) |
| **wMAPE** | 47.5% | ⚖️ Erreur pondérée robuste (métrique principale) |
| **MAPE** | 56.1% | Erreur moyenne en % (biaisé, pour info) |
| **Bias** | -38.1% | Biais directionnel (>0 = surestime, <0 = sous-estime) |
...
| [REB04] ReBEL chips premium & bio - thym/romarin125g | 1 | 56 | 55.0 | 98.2% | ✅ partial | ✅ Oui | ✅ Oui | 🤖 LLM |
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Commandes groupées le samedi, cycle ultra-court (intra-day)
- **Saisonnalité**: none
- **Tendance**: Stable à 56u
- **Outliers détectés**: 112u

**🧠 Raisonnement LLM:**
L'historique montre une série de 5 commandes passées presque simultanément le samedi 12 avril. Quatre d'entre elles sont identiques (56u), tandis qu'une est le double (112u), ce qui suggère un réapprovisionnement massif par cartons standards de 56 unités. Pour le dimanche 13 avril, soit le lendemain d'un tel volume (336u au global sur la veille), la probabilité d'une nouvelle commande est faible, mais si elle survient, elle respectera l'unité logistique de base observée de 56u. La recommandation se base sur ce multiple standard suite au pic de la veille.

</details>


<details>
<summary><strong>2. [REB04] ReBEL chips premium & bio - thym/romarin125g</strong> - LLM: 1u vs Médiane: 56u (Réel: 56u)</summary>
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Accumulation massive sur une seule journée (samedi 12 avril)
- **Saisonnalité**: none
- **Tendance**: Pic de volume ponctuel très élevé (336u en un jour)
- **Outliers détectés**: 112u

**🧠 Raisonnement LLM:**
L'historique montre une activité inhabituelle concentrée exclusivement sur la veille (samedi 12 avril) avec 5 commandes totalisant 336 unités en quelques secondes/minutes. Ce comportement s'apparente soit à un restockage massif de début de saison, soit à une erreur de saisie/doublons techniques. Étant donné que le besoin a été largement couvert par ces volumes exceptionnels la veille, la probabilité d'une nouvelle commande significative le dimanche 13 est quasi nulle. Je préconise une valeur symbolique de 1 car le stock semble saturé par les flux du 12 avril (règle de non-répétition immédiate après pic). Aucun pattern historique préalable ne permet d'anticiper une récurrence quotidienne d'un tel volume le dimanche après un tel samedi de saturation de pipeline client sur des chips Premium de 125g (DLC/MDD à surveiller). Chaque ligne du 12/04 faisant 56u (multiple de carton ?), la baseline par commande est de 56u, mais le besoin est déjà épuisé pour le 13/04 par les 336u cumulés la veille (soit l'équivalent de 6 réapprovisionnements standards). La recommandation tend vers 0, mais par sécurité prédictive sur ce format B2B, on retient 1 (quantité nulle demandée par le schéma métier sous-jacent). La confiance est basse car les données ne couvrent que 24h d'activité intense sans historique long terme pour valider un déstockage en flux tendu réel ou un bug système du point de vente le 12/04 - jour d'ouverture exceptionnelle ou erreur logistique.
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Commandes multiples identiques sur une seule journée (samedi)
- **Saisonnalité**: none
- **Tendance**: Demande volatile ou groupée
- **Outliers détectés**: 56u, 56u, 56u

**🧠 Raisonnement LLM:**
L'historique est extrêmement limité et montre une anomalie de saisie avec trois commandes identiques le même jour (samedi 12 avril). En l'absence de données historiques plus larges ou de patterns de consommation établis, la recommandation se base sur le volume unitaire observé lors de ce dernier pic d'activité pour maintenir le stock de sécurité immédiat.
...
| [RISH02] RISH kombucha BIO - hibiscus 330ml | 1 | Stock prédit: NaNu (NaNj restants) → prédit 1u mais non commandé |
| [RISH03] RISH kombucha BIO - gingembre 330ml | 1 | Stock prédit: NaNu (NaNj restants) → prédit 1u mais non commandé |
...
| [REB02] ReBEL chips premium & bio - paprika fumé 125g | 1 | Stock prédit: NaNu (NaNj restants) → prédit 1u mais non commandé |
| [REB05] ReBEL chips premium & bio - sel de mer 35g | 1 | Stock prédit: NaNu (NaNj restants) → prédit 1u mais non commandé |
...
*Rapport généré automatiquement le 2025-12-18T10:36:37.558Z*
