...
interface TestExample {
  ...
  currentDate: string;
  groundTruth: number; // Quantité réelle commandée
}

interface EvalResult {
  example: TestExample;
  baseline: { quantity: number; reasoning: string };
  mipro: { quantity: number; reasoning: string };
}
...
function formatOrders(orders: OrderItem[]): string {
  if (!orders || orders.length === 0) return "(Aucune commande)";
  return orders
    .map((o) => {
...
function extractFromV2File(filePath: string): TestExample[] {
...
function extractTestExamples(maxExamples: number): TestExample[] {
  ...
  const withOrder: TestExample[] = [];
  const withoutOrder: TestExample[] = [];

  for (const file of files) {
    for (const ex of extractFromV2File(join(REPORTS_DIR, file))) {
      if (ex.groundTruth > 0) withOrder.push(ex);
      else withoutOrder.push(ex);
    }
  }
...
// CALCUL MÉTRIQUES
// ============================================
function calcMetrics(results: Array<{ pred: number; actual: number }>): Metrics {
  let tp = 0,
    fp = 0,
    fn = 0,
    tn = 0;
...
async function main() {
  ...
  const baselineResults: Array<{ pred: number; actual: number }> = [];
  const miproResults: Array<{ pred: number; actual: number }> = [];
  ...
  console.log(
    `   Recall:    ${(baselineMetrics.recall * 100).toFixed(1)}% → ${(miproMetrics.recall * 100).toFixed(1)}% (${recallDelta >= 0 ? "+" : ""}${recallDelta.toFixed(1)}%)`
  );
  console.log(
    `   Precision: ${(baselineMetrics.precision * 100).toFixed(1)}% → ${(miproMetrics.precision * 100).toFixed(1)}% (${precisionDelta >= 0 ? "+" : ""}${precisionDelta.toFixed(1)}%)`
  );
  console.log(
    `   F1:        ${(baselineMetrics.f1 * 100).toFixed(1)}% → ${(miproMetrics.f1 * 100).toFixed(1)}% (${f1Delta >= 0 ? "+" : ""}${f1Delta.toFixed(1)}%)`
  );

  // Analyse des différences
  const improvements = results.filter(
    (r) => r.baseline.quantity > 0 && r.mipro.quantity === 0 && r.example.groundTruth === 0
  );
  const regressions = results.filter(
    (r) => r.baseline.quantity === 0 && r.mipro.quantity > 0 && r.example.groundTruth === 0
  );
  const missedByMipro = results.filter(
    (r) => r.baseline.quantity > 0 && r.mipro.quantity === 0 && r.example.groundTruth > 0
  );
