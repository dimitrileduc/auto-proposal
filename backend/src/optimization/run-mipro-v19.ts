...
// MÉTRIQUE SIMPLE
// ============================================
function stockMetric({ prediction, example }: any): number {
  ...
  if (!actual && predicted) return 0.3;
...
function extractFromV2File(filePath: string): TrainingExample[] {
...
function extractAndSplitExamples(reportsDir: string, maxExamples: number, valRatio = 0.2): DataSplit {
  ...
  const withOrder: TrainingExample[] = [];
  const withoutOrder: TrainingExample[] = [];

  for (const file of files) {
    for (const ex of extractFromV2File(join(reportsDir, file))) {
      if (ex.quantity > 0) withOrder.push(ex);
...
// ============================================
function calcMetrics(results: Array<{ pred: number; actual: number }>) {
...
async function main() {
  ...
  const baselineValResults: Array<{ pred: number; actual: number }> = [];
  ...
  const valResults: Array<{ pred: number; actual: number }> = [];
  for (const ex of valExamples) {
    try {
      const res = await stockPredictor.forward(studentLLM, {
        productName: ex.productName,
