...
- **Appels**: 41
- **Tokens**: 34,902 input + 11,239 output = 46,141 total
...
| Métrique | Valeur | Interprétation |
|----------|--------|----------------|
| **MAE** | 3.83 unités | Erreur moyenne absolue (symétrique) |
| **wMAPE** | 71.9% | ⚖️ Erreur pondérée robuste (métrique principale) |
| **MAPE** | 70.6% | Erreur moyenne en % (biaisé, pour info) |
| **Bias** | -71.9% | Biais directionnel (>0 = surestime, <0 = sous-estime) |
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Commande ponctuelle isolée ou cycle long imprécis
- **Saisonnalité**: none
- **Tendance**: Stable à 1u
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
L'historique est extrêmement limité avec une seule commande enregistrée il y a 7 semaines (49 jours). Bien que l'intervalle suggère un besoin non hebdomadaire, le volume unitaire consommé (1u) indique une rotation très faible pour ce produit bio infantile. En l'absence de données transactionnelles plus denses ou de saisonnalité N-1, la prédiction la plus probable pour maintenir le stock sans risque de péremption est le réapprovisionnement à l'unité identique à la dernière occurrence. Le respect du jour de la semaine (mercredi) renforce la probabilité d'une commande ce jour.
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Inconnu ou sporadiquement long (dernière commande il y a 8 semaines)
- **Saisonnalité**: none
- **Tendance**: Stable mais volume extrêmement faible
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
Avec une seule donnée historique datant de 8 semaines sur ce produit (LD Onion Spread 180g), il est impossible d'établir un cycle récurrent fiable. Cependant, la dernière commande était de 1 unité un mercredi, et la demande actuelle tombe également un mercredi. En l'absence de tendance haussière ou de données historiques N-1 suggérant une saisonnalité, la quantité la plus probable reste l'unité minimale de réapprovisionnement déjà observée.
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Rythme erratique et lent (6-8 semaines d'intervalle)
- **Saisonnalité**: none
- **Tendance**: Stable mais volume extrêmement faible (1-2 unités)
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
L'historique montre une demande très sporadique avec des volumes unitaires (1 ou 2 unités par commande). L'intervalle depuis la dernière commande (17 septembre) est de 8 semaines, ce qui correspond au cycle observé entre août et octobre 2024. Étant donné la faible vélocité du produit et l'absence de saisonnalité marquée en fin d'année sur ce format 250ml Weck, une commande de réapprovisionnement de 1 unité est la plus probable pour maintenir le stock sans sur-stockage.
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Rythme sporadique non-cyclique (intervalle de 45 à 60 jours)
- **Saisonnalité**: weak
- **Tendance**: Stable à environ 1-2u par commande
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
Le produit présente une demande faible mais constante. L'historique montre des commandes de 1 ou 2 unités environ tous les 1.5 à 2 mois. La dernière commande date du 17 septembre (il y a 8 semaines), ce qui correspond au cycle de réapprovisionnement observé en N-1 (ex: entre mars-avril ou avril-mai). La commande de novembre 2024 était de 2u à la même période. Compte tenu du délai depuis la dernière commande (septembre), un besoin de renouvellement de stock est imminent.
...
| [MF0030] MF Tarti Mangue Curry 250g  | 2 | Stock prédit: 2.2u (20j restants) → prédit 2u mais non commandé |
...
*Rapport généré automatiquement le 2025-12-18T11:36:39.598Z*
