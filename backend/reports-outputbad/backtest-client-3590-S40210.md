...
- **Appels**: 7
- **Tokens**: 11,443 input + 36,933 output = 48,376 total
...
| Métrique | Valeur | Interprétation |
|----------|--------|----------------|
| **MAE** | 105.80 unités | Erreur moyenne absolue (symétrique) |
...
| [PF0075] FILOU CHASSEUR  10 L | 88 | 264 | 176.0 | 66.7% | ✅ partial | ✅ Oui | ✅ Oui | 🤖 LLM |
...
**🔍 Analyse LLM:**
- **Pattern temporel**: weekly_with_variations
- **Saisonnalité**: none
- **Tendance**: increasing
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
Le jeudi n'est pas le jour principal de commande (lundi) mais est un jour secondaire avec des commandes historiques irrégulières. Pour minimiser le MAPE, on privilégie la valeur médiane et la plus fréquente (160u) plutôt que l'outlier récent (80u).
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Pattern hebdomadaire fort sur le lundi. Intervalle moyen entre lundis : 7 jours exactement. Présence occasionnelle de commandes jeudi considérées comme exceptionnelles (17,6% des cas). Double commande le même jour observée 2 fois (2024-06-17 et 2025-10-20), interprétée comme fractionnement/correction de commande du même client
- **Saisonnalité**: none
- **Tendance**: stabilité avec creux occasionnel
- **Outliers détectés**: -2025u

**🧠 Raisonnement LLM:**
Le jeudi 6 nov étant hors cycle, on prédit pour le prochain lundi 10 nov. Les volumes lundi récents (5u, 2u, 5u) montrent une stabilité autour de 4-5u. Le creux à 2u est un point bas occasionnel. La dernière valeur réelle (5u) est le meilleur indicateur pour une donnée limitée. L'outlier 15u du 23 oct est exclu (non répété). Aucun pattern mensuel ne justifie d'ajustement saisonnier. En agro B2B, viser la valeur la plus probable : 5u

</details>


<details>
<summary><strong>3. [PF0075] FILOU CHASSEUR  10 L</strong> - LLM: 88u vs Médiane: 132u (Réel: 264u)</summary>

**Quantités:**
- 🤖 **LLM prédit**: 88u (confidence: medium)
- 📊 **Baseline N-1**: 88u
- 📊 **Médiane**: 132u
- ✅ **Réel commandé**: 264u
- 📉 **Erreur LLM**: 176u (66.7%)
- 📉 **Erreur Médiane**: 132u (50.0%)

**🔍 Analyse LLM:**
- **Pattern temporel**: Bi-weekly Monday cycle with intermediate low-volume orders
- **Saisonnalité**: weak
- **Tendance**: slight decline
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
Le client a clairement migré vers un cycle de commande hebdomadaire le lundi avec deux niveaux distincts : 132u (commande principale) et 88u (réapprovisionnement intermédiaire). Le 27 octobre (dernier lundi) a été une commande principale de 132u. Le pattern récent (132u → 88u → 88u → 132u) indique que le prochain lundi devrait être un niveau intermédiaire de 88u. La date de prédiction étant un jeudi (hors du cycle habituel), j'applique la règle spéciale : prédire la quantité de la prochaine commande réelle, qui aura lieu le lundi 10 novembre.
...
**🧠 Raisonnement LLM:**
Conserver la valeur stable la plus récente (10u) issue du cycle mensuel octobre (N-1 vs N) tout en reconnaissant les pics saisonniers récurrents à 12u comme début de tendance haussière.
...
**✅ Quantité LLM**: 88u (confidence: medium)
...
*Rapport généré automatiquement le 2025-12-17T16:26:14.764Z*
