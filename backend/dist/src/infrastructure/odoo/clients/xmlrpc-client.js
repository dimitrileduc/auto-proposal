...
import { buildRecentOrdersDomain, buildInactivePartnersDomain, buildPartnerOrdersDomain, } from "./odoo-domains";
const ODOO_CONFIG = {
...
export function createXmlRpcClient() {
    ...
    return {
        async getInactiveCompanyPartners(dateMin, dateMax, excludeOrderTagId, excludedPartnerTagId) {
            try {
                // RPC 1: Récupérer les commandes récentes dans la période [dateMin, dateMax]
                const recentOrders = await odoo.searchRead("sale.order", buildRecentOrdersDomain(dateMin, dateMax, excludeOrderTagId), {
                    fields: ["partner_id"],
                });
                ...
                const inactivePartners = await odoo.searchRead("res.partner", buildInactivePartnersDomain(activePartnerIds, excludedPartnerTagId), {
        ...
        async getOrderHistoryByPartner(partnerId, windowDays, referenceDate, includeDraftOrders, excludedCategoryIds) {
            if (windowDays <= 0) {
            ...
            try {
                ...
                const orders = await odoo.searchRead("sale.order", buildPartnerOrdersDomain(partnerId, dateStart, states, referenceDate), {
                ...
                const domain = [["id", "in", orderLineIds]];
                ...
                const orderLines = await odoo.searchRead("sale.order.line", domain, {
                    fields: [
                        "id",
                        "product_id",
                        "product_uom_qty",
                ...
                throw error instanceof Error
                    ? error
                    : new Error(`Erreur lors de la récupération de l'historique du partenaire ${partnerId}: ${error}`);
            }
        },
        async getPartnerCompanyInfo(partnerId) {
            try {
                const result = await odoo.searchRead("res.partner", [["id", "=", partnerId]], { fields: ["name", "company_id"] });
                if (result.length === 0) {
                ...
                throw error instanceof Error
                    ? error
                    : new Error(`Erreur lors de la récupération des infos du partenaire ${partnerId}: ${error}`);
            }
        },
        ...
        async getSaleOrderDetails(quoteId) {
            try {
                // Récupérer le devis
                const orders = await odoo.searchRead("sale.order", [["id", "=", quoteId]], {
                    fields: [
                        "name",
                        "partner_id",
                        "company_id",
                ...
                const lines = await odoo.searchRead("sale.order.line", [["order_id", "=", quoteId]], {
                    fields: [
                        "order_id",
                        "product_id",
                        "product_uom",
                ...
                throw error instanceof Error
                    ? error
                    : new Error(`Erreur lors de la récupération des détails du devis ${quoteId}: ${error}`);
            }
        },
