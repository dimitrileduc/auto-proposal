...
export const orchestratorTask = task({
    ...
    run: async (payload) => {
        ...
        const config = {
            ...
            generateReports: payload.config?.generateReports ??
                autoProposalConfig.workflow.generateReports,
            excludedPartnerTagId: payload.config?.excludedPartnerTagId ??
        ...
        try {
            ...
            const allInactiveClients = await getInactiveClients(config.dateMin, config.dateMax, config.forceReanalysis ? autoProposalConfig.quoteGeneration.autoProposalTagId : undefined, config.excludedPartnerTagId);
                    ...
                    for (const run of batchResults.runs) {
                        if (run.ok) {
                            ...
                            if (taskResult.summary.quoteName) {
                                ...
                                console.log(`   ✅ ${taskResult.client.name}: ${taskResult.summary.productsCount} products, ` +
            ...
            const statistics = calculateGlobalWorkflowStatistics(allInactiveClients, clientResults);
            ...
            const inactivityDaysForReport = Math.round((new Date(config.dateMax).getTime() - new Date(config.dateMin).getTime()) / (1000 * 60 * 60 * 24));
