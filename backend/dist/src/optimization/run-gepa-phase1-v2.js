...
function recallPriorityMetric({ prediction, example }) {
    metricCallCount++;
    ...
    if (prediction?.willOrder === undefined || prediction?.willOrder === null) {
        if (metricCallCount <= 5)
            console.log(`[METRIC ${metricCallCount}] → willOrder undefined, score=0`);
        return 0;
    }
...
// ============================================================================
function feedbackFn({ prediction, example }) {
    const predicted = prediction?.willOrder === true;
    const actual = example.willOrder === true || example.quantity > 0;
    if (actual && !predicted) {
...
async function main() {
    ...
    const result = await optimizer.compile(classifier, axTrainExamples, recallPriorityMetric, // Métrique SCALAIRE (retourne number, pas object)
    {
