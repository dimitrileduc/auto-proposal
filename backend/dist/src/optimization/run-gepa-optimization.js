...
import { gepaMultiObjectiveMetric, gepaCombinedMetric, calculateAggregateMetrics, } from "./gepa-metric.js";
// Configuration
...
`;
    ...
    const result = await optimizer.compile(stockPredictor, axTrainExamples, metricFn, {
    ...
    if (!meetsRecall || !meetsPrecision || !meetsWMAPE) {
        ...
        if (!meetsRecall)
            console.warn("   - Augmenter NUM_TRIALS ou ajuster poids recall");
        if (!meetsPrecision)
            console.warn("   - Vérifier diversité examples (FP rate)");
        if (!meetsWMAPE)
            console.warn("   - Augmenter poids accuracy dans métrique");
    }
    // 9. Sauvegarder
    const savedData = {
        version: "2.0",
