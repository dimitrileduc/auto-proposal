...
function analyzeOrderCycles() {
    const data = JSON.parse(readFileSync("./analysis-folder/predictions-v2-all-clients.json", "utf-8"));
    // Pour les produits avec 1 seule commande en 90j, quand était la précédente?
    const gapToPreviousOrder = [];
        ...
        for (const product of allProducts) {
            ...
            for (const windowDays of windows) {
                ...
                if (ordersInWindow.length === 0)
                    windowResults[windowDays].zero++;
                else if (ordersInWindow.length === 1)
                    windowResults[windowDays].one++;
                else
                    windowResults[windowDays].twoPlus++;
            }
    ...
    for (const w of windows) {
        ...
        console.log(`| ${w.toString().padStart(3)}j    | ` +
