...
async function runComparison(optimizerName, filePath, testSet) {
    console.log(`\n${optimizerName}...`);
    ...
    try {
        ...
        if (data.optimizedProgram) {
            ...
            if (data.instruction)
                predictor.setInstruction(data.instruction);
            if (data.demos)
                predictor.setDemos(data.demos);
        }
    }
    catch (error) {
...
async function main() {
    ...
    console.log("=".repeat(60));
    // Charger test set
    const valExamples = JSON.parse(readFileSync("./optimization-results/training-examples-gepa-val.json", "utf-8"));
    const testSet = valExamples.slice(0, 100);
    console.log(`\nTest: ${testSet.length} exemples`);
    // Comparaisons
    const baseline = await runComparison("Baseline", "", testSet);
    const mipro = await runComparison("MiPRO", "./optimization-results/stock-predictor-mipro.json", testSet);
    const gepa = await runComparison("GEPA", "./optimization-results/stock-predictor-gepa.json", testSet);
    // Tableau
    console.log("\n" + "=".repeat(60));
    ...
    console.log("|------------|----------|----------|----------|----------|");
    const formatPct = (val) => val ? `${(val * 100).toFixed(1)}%`.padStart(6) : "  N/A  ";
    const formatNum = (val) => val ? `${val.toFixed(1)}%`.padStart(6) : "  N/A  ";
    console.log(`| Recall     | ${formatPct(baseline?.recall)} | ${formatPct(mipro?.recall)} | ${formatPct(gepa?.recall)} | ≥90%     |`);
    console.log(`| Precision  | ${formatPct(baseline?.precision)} | ${formatPct(mipro?.precision)} | ${formatPct(gepa?.precision)} | ≥40%     |`);
    console.log(`| F1         | ${formatPct(baseline?.f1)} | ${formatPct(mipro?.f1)} | ${formatPct(gepa?.f1)} | -        |`);
    console.log(`| WMAPE      | ${formatNum(baseline?.wmape)} | ${formatNum(mipro?.wmape)} | ${formatNum(gepa?.wmape)} | ≤25%     |`);
    // Verdict
    const gepaOK = gepa && gepa.recall >= 0.9 && gepa.precision >= 0.4 && gepa.wmape <= 25.0;
    const miproOK = mipro && mipro.recall >= 0.9 && mipro.precision >= 0.4 && mipro.wmape <= 25.0;
    console.log("\n📊 VERDICT:");
    if (gepaOK && !miproOK) {
