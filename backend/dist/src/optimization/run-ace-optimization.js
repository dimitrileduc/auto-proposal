...
function extractFromProduct(product, cutoffDate, type) {
...
function formatPlaybookForPrompt(playbook) {
    if (!playbook?.sections)
        return "";
    const lines = ["RÈGLES DÉCOUVERTES PAR ACE:"];
...
function calculateMetrics(results) {
    ...
    for (const { pred, actual } of results) {
        ...
        if (predOUI && actualOUI)
            tp++;
        else if (predOUI && !actualOUI)
            fp++;
        else if (!predOUI && actualOUI)
            fn++;
        else
...
// 3. Baseline
async function runBaseline(predictor, examples) {
...
// 4. ACE Optimization
async function runACE(examples) {
    ...
    const aceMetric = ({ prediction, example }) => {
        const pred = (prediction.ruptureRisk || "NON").toString().toUpperCase();
        const actual = (example.ruptureRisk || "NON").toString().toUpperCase();
        const predOUI = pred === "OUI";
        const actualOUI = actual === "OUI";
        // Métrique simple: match exact
        if (predOUI === actualOUI)
            return 1; // Correct (TP ou TN)
    ...
    try {
        ...
        try {
            ...
            playbook = ace.playbook || ace._playbook || {};
            instruction = ace.currentInstruction || null;
...
async function main() {
    ...
    writeFileSync(join(OUTPUT_DIR, "training-examples-ace.json"), JSON.stringify(examples, null, 2));
    // 2. Baseline
    ...
    console.log(`\n         | Baseline | ACE      | Delta`);
    console.log(`---------+----------+----------+-------`);
    console.log(`Recall   | ${(baseline.recall * 100).toFixed(1).padStart(6)}%  | ${(aceMetrics.recall * 100).toFixed(1).padStart(6)}%  | ${((aceMetrics.recall - baseline.recall) * 100).toFixed(1)}%`);
    console.log(`Precision| ${(baseline.precision * 100).toFixed(1).padStart(6)}%  | ${(aceMetrics.precision * 100).toFixed(1).padStart(6)}%  | ${((aceMetrics.precision - baseline.precision) * 100).toFixed(1)}%`);
    console.log(`F1       | ${(baseline.f1 * 100).toFixed(1).padStart(6)}%  | ${(aceMetrics.f1 * 100).toFixed(1).padStart(6)}%  | ${((aceMetrics.f1 - baseline.f1) * 100).toFixed(1)}%`);
    ...
    writeFileSync(join(OUTPUT_DIR, "ace-playbook.json"), JSON.stringify(savedData, null, 2));
