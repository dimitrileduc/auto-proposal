...
`;
function classificationMetric({ prediction, example }) {
...
async function main() {
    ...
    const result = await optimizer.compile(classifier, axTrainExamples, classificationMetric, {
    ...
    if (result.paretoFront && result.paretoFront.length > 0) {
        ...
        for (let i = 0; i < result.paretoFront.length; i++) {
            ...
            console.log(`  Instruction: ${solution.instruction?.substring(0, 80) || "N/A"}...`);
            console.log();
        }
