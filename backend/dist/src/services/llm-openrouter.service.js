...
function loadOptimizedDemos() {
    ...
    try {
        ...
        const demosWithQty = traces.filter((t) => t.quantity > 0).slice(0, 3);
        const demosWithoutQty = traces.filter((t) => t.quantity === 0).slice(0, 2);
 ...
 */
export async function predictWithLLM(input) {
    ...
    const formatOrderWithWeekday = (order) => {
...

{
  ...
  "reasoning": "explication concise"
    ...
    try {
        ...
        if (isClaudeModel && choice?.message?.tool_calls) {
            ...
            try {
                ...
                try {
                    ...
                    prediction = JSON.parse(sanitized);
                }
                catch (e) {
        ...
        throw new Error(`Failed to predict with LLM: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
}
