 ...
 */
export function filterLowConfidence(comparison) {
    ...
    const productMetrics = calculateProductMetrics(filteredTP.length, comparison.falsePositives.length, comparison.falseNegatives.length);
 ...
 */
export function filterOnlyLowConfidence(comparison) {
 ...
 */
function generateLLMInputDataSection(truePositives) {
...
${recentOrders.length > 0
            ? recentOrders.map((o) => `- ${o.date}: ${o.quantity}u`).join('\n')
            : '- Aucune commande récente'}

**📅 Commandes N-1 (même période année dernière):**
${lastYearOrders.length > 0
            ? lastYearOrders.map((o) => `- ${o.date}: ${o.quantity}u`).join('\n')
            : '- Aucune commande N-1'}
 ...
 */
function generateLLMDetailSection(truePositives) {
...
- **Outliers détectés**: ${tp.llmPrediction.analysis?.detected_outliers && tp.llmPrediction.analysis.detected_outliers.length > 0
            ? tp.llmPrediction.analysis.detected_outliers.map((o) => `${o}u`).join(', ')
            : 'Aucun'}
 ...
 */
export function generateBacktestReport(comparison) {
...
|---------|--------|------|-----------|----------|------|------------|------------|--------|
${comparison.truePositives.map(tp => `| ${tp.productName} | ${tp.predictedQty} | ${tp.realQty} | ${tp.absoluteError.toFixed(1)} | ${tp.errorPercent.toFixed(1)}% | ${getMatchTypeEmoji(tp.matchType)} ${tp.matchType} | ${tp.llm_required ? '✅ Oui' : '❌ Non'} | ${tp.llm_success ? '✅ Oui' : '❌ Non'} | ${tp.quantitySource === 'llm' ? '🤖 LLM' : '📊 Médiane'} |`).join('\n')}
...
|---------|-------------|--------|
${comparison.falsePositives.map(fp => `| ${fp.productName} | ${fp.qty} | ${fp.reason} |`).join('\n')}
...
|---------|---------------|--------|
${comparison.falseNegatives.map(fn => `| ${fn.productName} | ${fn.qty} | ${fn.reason} |`).join('\n')}
 ...
 */
function calculateRMSE(truePositives) {
...
function calculateDaysAgo(dateString, referenceDate) {
    const date = new Date(dateString);
    const ref = referenceDate ? new Date(referenceDate) : new Date();
    return Math.floor((ref.getTime() - date.getTime()) / (1000 * 60 * 60 * 24));
}
function detectTrend(quantities) {
...
function detectSeasonality(dates) {
    if (dates.length < 4)
        return false;
...
function calculateRegularityScore(orders, cv) {
    if (orders.length < 2)
        return 0;
 ...
 */
function classifyError(predictedQty, realQty, absoluteError, errorPercent) {
    const direction = predictedQty > realQty ? 'over' : predictedQty < realQty ? 'under' : 'exact';
    const matchType = absoluteError === 0 ? 'exact' : 'partial';
    let severity;
    if (absoluteError === 0)
        severity = 'none';
    else if (errorPercent <= 10 || absoluteError <= 2)
        severity = 'low';
    else if (errorPercent <= 25 || absoluteError <= 5)
        severity = 'medium';
    else if (errorPercent <= 50 || absoluteError <= 10)
        severity = 'high';
    else
        severity = 'critical';
    return { direction, severity, matchType };
}
function classifyMismatch(mismatchType, reason, quantity) {
    let category;
    if (reason.includes('Stock suffisant') || reason.includes('stock:'))
        category = 'stock_sufficient';
    else if (reason.includes('Jamais commandé') || reason.includes("pas d'historique"))
        category = 'no_history';
    else if (reason.includes('LLM') || reason.includes('llm'))
        category = 'llm_error';
    else if (reason.includes('filtré') || reason.includes('seuil'))
        category = 'threshold_filtered';
    else
        category = 'other';
    let severity;
 ...
 */
function calculateHistoryStatistics(quantities, orders) {
 ...
 * Helpers de features
 */
function calculateProductFeatures(orders, statistics, cutoffDate) {
    if (orders.length === 0) {
        return {
    ...
    const quantityType = statistics.cv < 0.15 ? 'fixed' : statistics.cv < 0.5 ? 'variable' : 'highly_variable';
    const orderingPattern = statistics.regularityScore > 0.7 ? 'regular' : statistics.regularityScore > 0.4 ? 'seasonal' : 'irregular';
    const dates = orders.map(o => new Date(o.date_order)).sort((a, b) => a.getTime() - b.getTime());
...
function enrichProductWithFeatures(product, analyzedProduct, cutoffDate) {
 ...
 */
function calculateMetricsForConfidenceLevel(comparison, confidenceLevel) {
    ...
    const productMetrics = calculateProductMetrics(filteredTP.length, filteredFP.length, filteredFN.length);
...
function calculateMetricsByConfidence(comparison) {
    // Métriques globales
    const all = {
        counts: {
 ...
 */
export function generateBacktestReportJSONv2(comparison, stockAnalysis) {
    // === META ===
    const meta = {
        version: "2.0.0",
    ...
    const analyzedProductsMap = new Map(allProducts.map((p) => [p.product_id, p]));
    // === ENRICH TRUE POSITIVES ===
    const enrichedTP = comparison.truePositives.map((tp) => {
        const analyzedProduct = analyzedProductsMap.get(tp.productId);
        // Error classification
        const errorClassification = classifyError(tp.predictedQty, tp.realQty, tp.absoluteError, tp.errorPercent);
        // History + statistics + features
        const history = enrichProductWithFeatures(tp, analyzedProduct, comparison.cutoffDate);
        // LLM data
        const llm = tp.llm_required
            ? {
                required: true,
        ...
        if (llm && tp.llmPrediction) {
            ...
            if (tp.medianQty !== undefined) {
                ...
                const llmImprovement = medianError > 0 ? ((medianError - llmError) / medianError) * 100 : 0;
                llm.comparison = {
        ...
        const stock = analyzedProduct
            ? {
                ...
                replenishmentThreshold: analyzedProduct.calculation_metadata?.minimum_qty_threshold || 0,
                consumptionPerDay: analyzedProduct.stock_prediction?.consumption_per_day || 0,
            }
            : undefined;
        return {
    ...
    const enrichedFP = comparison.falsePositives.map((fp) => {
        ...
        const llm = analyzedProduct?.llm_required
            ? {
                required: true,
    ...
    const enrichedFN = comparison.falseNegatives.map((fn) => {
 ...
 */
export function generateAggregatedReportV2(reportsV2) {
    if (reportsV2.length === 0) {
        throw new Error("Aucun rapport v2 à agréger");
    }
    // Agréger les counts
    let totalTP = 0, totalFP = 0, totalFN = 0;
    let totalPromptTokens = 0, totalCompletionTokens = 0, totalCalls = 0;
    ...
    let totalMAE = 0, totalWMAPE = 0, totalMAPE = 0, totalBias = 0, totalRMSE = 0, exactMatchCount = 0, partialMatchCount = 0;
    reportsV2.forEach((report) => {
    ...
    ["low", "medium", "high"].forEach((conf) => {
        ...
        const confF1 = confPrecision + confRecall > 0
 ...
 */
export function generateBacktestReportJSON(comparison) {
    return {
 ...
 */
export function generateSummaryMarkdown(reportsV2) {
    if (reportsV2.length === 0) {
