 ...
 */
export function calculateGlobalWorkflowStatistics(allInactiveClients, clientResults) {
    ...
    const clientsWithOrderHistory = clientResults.filter(r => r.success && r.phases.stockAnalysis && r.phases.stockAnalysis.total_products_in_history > 0).length;
    // Clients avec historique MAIS sans risque de rupture
    // = ceux qui ont un historique MAIS hasRisk === false
    const clientsWithoutRisk = clientResults.filter(r => r.success && r.phases.stockAnalysis && r.phases.stockAnalysis.total_products_in_history > 0 && !r.hasRisk).length;
