 ...
 */
export function isWithinLastNMonths(orderDate, referenceDate, months) {
 ...
 */
export function isSamePeriodLastYear(orderDate, referenceDate, _months // Ignored, kept for compatibility
) {
    const order = new Date(orderDate);
    const reference = new Date(referenceDate);
 ...
 */
export function splitOrdersByPeriod(orders, currentDate, months = 3, maxOrdersPerView = 12 // Limited to 12 to prevent LLM confusion (IRIS article: "LLMs confuse with >3 weeks data")
) {
