...
async function analyzeProductHistory() {
    ...
    try {
        ...
        const products = await odooClient.execute('product.product', 'search_read', [[['default_code', 'in', FP_PRODUCT_CODES]]], { fields: ['id', 'default_code', 'name'] });
        ...
        const orderLinesBeforeWindow = await odooClient.execute('sale.order.line', 'search_read', [[
        ...
        const orderLinesInWindow = await odooClient.execute('sale.order.line', 'search_read', [[
                ['order_partner_id', '=', CLIENT_ID],
                ['product_id', 'in', productIds],
                ['order_id.date_order', '>=', WINDOW_START],
        ...
        const allOrderIds = [
            ...orderLinesBeforeWindow.map((l) => l.order_id[0]),
            ...orderLinesInWindow.map((l) => l.order_id[0]),
        ];
        const uniqueOrderIds = [...new Set(allOrderIds)];
        const orders = await odooClient.execute('sale.order', 'read', [uniqueOrderIds], { fields: ['id', 'name', 'date_order'] });
        ...
        orderLinesBeforeWindow.forEach((line) => {
        ...
        orderLinesInWindow.forEach((line) => {
            const product = productMap.get(line.product_id[0]);
            if (!product)
                return;
            const stats = productStats.get(product.default_code);
        ...
        productStats.forEach((stats, code) => {
            if (stats.commandedBeforeWindow)
                fpCommandedBeforeWindow++;
            if (stats.commandedInWindow)
                fpCommandedInWindow++;
            if (!stats.commandedBeforeWindow && !stats.commandedInWindow)
                fpNeverCommanded++;
        });
