...
- **Appels**: 9
- **Tokens**: 7,829 input + 2,590 output = 10,419 total
...
| **wMAPE** | 125.0% | ⚖️ Erreur pondérée robuste (métrique principale) |
| **MAPE** | 133.3% | Erreur moyenne en % (biaisé, pour info) |
| **Bias** | 125.0% | Biais directionnel (>0 = surestime, <0 = sous-estime) |
...
**🧠 Raisonnement LLM:**
Étape 1 (De-eventing) : Aucune donnée ne permet de qualifier la commande du 11/08 d'outlier. Étape 2 (Saisonnalité) : Absence de données N-1 pour confirmer un impact saisonnier. Étape 3 (Tendance) : Historique quasi inexistant (une seule commande en 3 mois), la tendance ne peut être calculée. Étape 4 (Recommandation) : En l'absence de données significatives et par principe de conservation en supply chain B2B, on se base sur le seul point de donnée connu (3 unités) pour assurer une continuité minimale sans risquer de sur-stockage sur une référence à faible rotation.
...
**🧠 Raisonnement LLM:**
Étape 1: Aucune donnée aberrante détectée sur l'unique point de données de 2u. Étape 2: Absence de données N-1 empêchant une analyse de saisonnalité fiable, bien que la confiture d'abricot ne présente généralement pas de pic majeur en novembre. Étape 3: Le rythme actuel est très espacé (dernière commande il y a 84 jours), suggérant une rotation lente ou un achat d'appoint. Étape 4: En l'absence de récurrence établie et pour éviter le sur-stockage sur un produit à faible rotation, la recommandation se cale sur le dernier volume observé (2u). Le niveau de confiance est faible en raison de la pauvreté de l'historique.
...
*Rapport généré automatiquement le 2025-12-18T12:14:27.839Z*
