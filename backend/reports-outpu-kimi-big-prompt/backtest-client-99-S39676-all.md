...
- **Appels**: 37
- **Tokens**: 53,404 input + 108,713 output = 162,117 total
...
| [TEN01] TENSAÏ TEA  thé vert bio au citron et à la fleur de sureau 330ml | 5 | 1 | 4.0 | 400.0% | ✅ partial | ✅ Oui | ✅ Oui | 🤖 LLM |
...
**🔍 Analyse LLM:**
- **Pattern temporel**: sporadic_hebdo_mixed
- **Saisonnalité**: none
- **Tendance**: stable_low_volume
- **Outliers détectés**: 2u

**🧠 Raisonnement LLM:**
Historique montre commandes très éparses mais volume stable à 1u sur les 3 derniers mois. Le 2u de N-1 est un outlier isolé non répété. Le lundi est un jour de commande possible. La quantité la plus probable reste 1u.
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Aucun cycle régulier identifié (intervalle de 38 jours entre les 2 dernières commandes)
- **Saisonnalité**: none
- **Tendance**: slight_decrease
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
Le client commande de manière épisodique avec un intervalle moyen de 38 jours, sans cycle hebdomadaire ou mensuel établi. La dernière commande du 9 octobre (1u) constitue le seul indicateur fiable de l'activité récente. Prédire 2u (dernier lundi) ignorerait la tendance à la baisse observée. Prédire 1,5u (moyenne) lisserait l'information insuffisante. La règle anti-zéro interdit toute prédiction nulle. En l'absence de pattern, la dernière valeur connue (1u) minimise le MAPE potentiel.
...
**🔍 Analyse LLM:**
- **Pattern temporel**: No regular week day pattern identified: the three orders fall on different weekdays (Friday, Monday, Thursday) with irregular intervals of 45 and 38 days. The cadence is closer to a sporadic ~40-day cycle rather than weekly.
- **Saisonnalité**: none
- **Tendance**: stable
- **Outliers détectés**: Aucun
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Intervalles irréguliers (18j, 27j, 21j) suggérant un rythme mensuel épars plutôt que hebdomadaire
- **Saisonnalité**: none
- **Tendance**: stable
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
**ANALYSE RYTHMIQUE** : Pattern atypique avec 4 commandes sur 3 mois (intervalles 18-27j). Pas de cycle hebdomadaire strict. Le lundi est un jour de commande *récent* (2/4 dernières) mais pas exclusif. La date 2025-10-13 (lundi) tombe 21j après la dernière commande → intervalle parfaitement cohérent avec l'historique, donc **une commande est attendue**.

**OUTLIERS** : Aucun. Les volumes 1u/2u sont extrêmement proches et stables (écart <2×).

**SYNTHÈSE** : Données très limitées sans saisonnalité N-1. Les 2 dernières commandes lundi montrent une légère hausse (1→2u). En B2B agroalimentaire, avec si peu de points, la **récency prime** : la dernière observation sur jour similaire (lundi) est le meilleur proxy. L'intervalle de 21j confirme que le timing est adéquat pour une nouvelle commande. Prédire 1u serait sous-estimer la dynamique récente sans réduction de risque MAPE (erreur potentielle 100% vs 50%).
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Pattern de 21 jours récent (3 semaines) sur lundi
- **Saisonnalité**: none
- **Tendance**: stable
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
Les 2 dernières commandes sont des lundis de 2u espacées de 21 jours. La date de prédiction tombe exactement sur ce cycle (21 jours après le 2025-09-22). La stabilité parfaite (3×2u) et la cohérence du jour de la semaine rendent 2u la prédiction la plus probable.
...
**🔍 Analyse LLM:**
- **Pattern temporel**: NON DÉFINI - Aucune donnée disponible pour calculer les intervalles moyens entre commandes.
- **Saisonnalité**: none
- **Tendance**: Impossible à évaluer - absence de points de référence
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
Pas de données historiques disponibles - N-1 et période récente sont vides. En supply chain B2B, cette absence totale est l'indicateur le plus fort d'un produit sans activité prévisible. La règle 'JAMAIS 0 pour jour hors cycle' ne s'applique pas car le cycle est inconnu. La prédiction la plus précise (minimisation du MAPE) est donc 0, correspondant à l'hypothèse d'un produit inactive ou jamais commandé.

</details>


<details>
<summary><strong>7. [TEN01] TENSAÏ TEA  thé vert bio au citron et à la fleur de sureau 330ml</strong> - LLM: 5u vs Médiane: 1u (Réel: 1u)</summary>
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Non déterminable (0 point de données). Le client est soit nouveau, soit le produit vient d'être lancé, soit l'historique n'a pas été remonté.
- **Saisonnalité**: none
- **Tendance**: stable
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
**Absence totale de données historiques** interdit tout calcul statistique. Le lundi étant un jour ouvré classique de planification B2B, et non un week-end (où 0 serait probable), je privilégie l'hypothèse d'une **commande de routine potentielle**. Pour un produit niche bio 330ml, une première ou routine commande B2B adopte une quantité test standard. La valeur 5u est choisie comme représentative d'un palier minimal de commande raisonnable en B2B agroalimentaire, respectant la règle d'or: valeur la plus probable sans surgonflement, évitant le MAPE catastrophique d'une prédiction 0 si le client commande effectivement ce jour.
...
| [TEN04] TENSAÏ TEA  thé matcha bio à la menthe 330ml | 0 | Stock prédit: 0.0u (2j restants) → prédit 0u mais non commandé |
...
| [LEA04] LEAMO ginger beer bio 750ml | 12 | Stock prédit: -0.7u (-46j restants) → prédit 12u mais non commandé |
...
*Rapport généré automatiquement le 2025-12-18T08:41:34.146Z*
