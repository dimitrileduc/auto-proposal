...
- **Appels**: 7
- **Tokens**: 11,443 input + 27,457 output = 38,900 total
...
| Métrique | Valeur | Interprétation |
|----------|--------|----------------|
| **MAE** | 108.40 unités | Erreur moyenne absolue (symétrique) |
| **wMAPE** | 68.3% | ⚖️ Erreur pondérée robuste (métrique principale) |
| **MAPE** | 49.8% | Erreur moyenne en % (biaisé, pour info) |
...
| [PF0075] FILOU CHASSEUR  10 L | 156 | 264 | 108.0 | 40.9% | ✅ partial | ✅ Oui | ✅ Oui | 🤖 LLM |
...
**🧠 Raisonnement LLM:**
Analyse rythmique : Pattern bi-mensuel (14j) identifié sur les jeudi avec dernière commande le 2025-10-23 (80u). La date de prédiction (2025-11-06) correspond exactement au cycle suivant (+14j). Le pattern lundi hebdomadaire est stable (240u) mais représente un rythme différent. Le volume récent du jeudi (80u) est bien inférieur à l'historique N-1 (160-240u), mais constitue la seule donnée fiable pour ce cycle. Pas d'outliers à exclure : la valeur 80u est la baseline actuelle du jeudi. En B2B, privilégier la plus récente donnée spécifique au jour plutôt que dextrapoler depuis un pattern différent (lundi).
...
**🧠 Raisonnement LLM:**
Le client présente un cycle hebdomadaire principal les **lundis** (80% des commandes) avec des volumes récents stables à 4-5 unités (moyenne pondérée des 3 derniers lundis = 4u). Les commandes de **jeudi** sont occasionnelles (20% de fréquence) et irrégulières : historique [2, 4, 10, 15] avec une variance élevée. La dernière commande jeudi (23 oct. 2025) à 15u est un **outlier manifeste** : volume 3× supérieur aux commandes lundi adjacentes (5u et 2u), sans pattern bi-mensuel établi ni rupture de tendance confirmée (point isolé). La date de prédiction (6 nov.) est un jeudi hors cycle principal. D'après les règles spécifiques, on ne prédit pas 0 mais la quantité la **plus probable** pour une commande incidente : médiane des valeurs typiques client (2-5u) = **4 unités**. Ce choix minimise le MAPE en cas de non-commande (sur-prédiction contrôlée) et reste dans la fourchette des volumes récurrents observés.

</details>


<details>
<summary><strong>3. [PF0075] FILOU CHASSEUR  10 L</strong> - LLM: 156u vs Médiane: 132u (Réel: 264u)</summary>

**Quantités:**
- 🤖 **LLM prédit**: 156u (confidence: high)
- 📊 **Baseline N-1**: 156u
- 📊 **Médiane**: 132u
- ✅ **Réel commandé**: 264u
- 📉 **Erreur LLM**: 108u (40.9%)
- 📉 **Erreur Médiane**: 132u (50.0%)

**🔍 Analyse LLM:**
- **Pattern temporel**: Toutes les semaines (lundi)
- **Saisonnalité**: strong
- **Tendance**: increasing
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
Le produit PF0075 présente un pattern hebdomadaire très clair avec une alternance stabe entre 156u (vendredi) et 104u (lundi). Les commandes récentes du vendredi suivent exactement ce pattern avec 156u. La prédiction pour vendredi 2025-11-07 maintient cette valeur fiable de 156u car : 1) Pattern hebdomadaire établi depuis N-1 2) Aucune rupture de tendance détectée 3) Les données récentes confirment la stabilité du cycle vendredi. Le client B2B suit un rythme opérationnel rigoureux qu'il n'a pas modifié.
...
**🧠 Raisonnement LLM:**
Given the irregular intervals and limited data, the prediction weighs the most recent command (12u) more heavily while calibrating for the typical Thursday baseline (6u). The outlier 20u is treated as a one-off event and excluded. The resulting 10u reflects the upward trend observed in recent data while staying aligned with historical Thursday patterns. The modest confidence score of 0.72 accounts for data sparsity and high variability.
...
**✅ Quantité LLM**: 156u (confidence: high)
...
| [PF3290] JF MAYO BARAKI 925ML | 0 | Stock prédit: -0.8u (-48j restants) → prédit 0u mais non commandé |
...
*Rapport généré automatiquement le 2025-12-18T08:09:53.674Z*
