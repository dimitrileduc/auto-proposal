...
- **Appels**: 42
- **Tokens**: 64,047 input + 128,349 output = 192,396 total
...
| Métrique | Valeur | Interprétation |
|----------|--------|----------------|
| **MAE** | 13.25 unités | Erreur moyenne absolue (symétrique) |
| **wMAPE** | 82.8% | ⚖️ Erreur pondérée robuste (métrique principale) |
| **MAPE** | 70.9% | Erreur moyenne en % (biaisé, pour info) |
| **Bias** | -82.8% | Biais directionnel (>0 = surestime, <0 = sous-estime) |
...
| [JF012] JF SAUCE BEARNAISE 250ML WECK | 2 | 21 | 19.0 | 90.5% | ✅ partial | ✅ Oui | ✅ Oui | 🤖 LLM |
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Aperiodic - No stable weekly or monthly cycle identified
- **Saisonnalité**: weak
- **Tendance**: stable to slightly decreasing
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
Client shows irregular ordering pattern with no fixed day. While recent order was 1u, historical data shows 2u as the most frequent volume (4/10 orders). The 6u spike is an outlier. Given B2B agro context and need to minimize MAPE, selecting the modal value 2u is safer than overfitting to the latest isolated data point. The 22-day interval since last order (Oct 21) makes a mid-November order plausible.

</details>


<details>
<summary><strong>2. [JF012] JF SAUCE BEARNAISE 250ML WECK</strong> - LLM: 2u vs Médiane: 8u (Réel: 21u)</summary>
...
**🔍 Analyse LLM:**
- **Pattern temporel**: Cyclic irregular
- **Saisonnalité**: none
- **Tendance**: Unknown
- **Outliers détectés**: Aucun

**🧠 Raisonnement LLM:**
Analysis of order history reveals irregular intervals (7-39 days) with no consistent weekly pattern. The most recent order (2025-09-23) shows an outlier quantity of 10u (10× previous order), which appears exceptional and non-recurring. Excluding this anomaly, baseline values cluster around 1-4u with a median of 2u. The prediction date falls outside recent ordering days (Tuesday/Friday), triggering special handling: instead of predicting zero, the model forecasts the next probable order quantity. Given the 48-day gap since the last data point and the product's premium nature, a conservative 2u reflects the most statistically probable standard order volume while accounting for potential replenishment needs.
...
| [JF001] JF MAYONNAI TRUFFES 250ML WECK | 1 | Stock prédit: -2.2u (-7j restants) → prédit 1u mais non commandé |
...
*Rapport généré automatiquement le 2025-12-18T08:13:16.438Z*
